...
import { drawGiveawayWinners, GiveawayError } from "@/lib/server/repositories/giveaway-repository"
...
export const POST = withV2Params<{ uuid: string }>(async (ctx, { uuid }) => {
  ...
  try {
    const winners = await drawGiveawayWinners(ctx.db, uuid, user, Math.floor(Date.now() / 1000))

    await insertAuditLog(ctx.db, "giveaway_drawn", "giveaway", uuid, {
      actor: user,
