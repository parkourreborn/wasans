...
import { TrialLifecycleError, unretireTrial } from "@/lib/server/repositories/trial-repository"
...
export const POST = withV2Params<{ name: string }>(async (ctx, { name }) => {
  ...
  try {
    await unretireTrial(ctx.db, trialName)
  } catch (error) {
  ...
  await insertAuditLog(ctx.db, "trial_unretired", "trial", trialName, {
    actor: user,
