...
import { PrizeError, rejectPrizeCandidate } from "@/lib/server/repositories/prize-repository"
...
export const POST = withV2Params<{ uuid: string }>(async (ctx, { uuid }) => {
  ...
  try {
    const candidate = await rejectPrizeCandidate(ctx.db, uuid, user, Math.floor(Date.now() / 1000))

    await insertAuditLog(ctx.db, "prize_candidate_rejected", "prize_candidate", uuid, {
      actor: user,
      targetType: "prize",
      targetUuid: candidate.prize_uuid,
    })
    await bumpCacheGeneration(ctx.cache)

    return jsonOk(candidate, { requestId: ctx.requestId })
  } catch (error) {
