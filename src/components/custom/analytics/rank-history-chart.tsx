...
export function RankHistoryChart({ rows }: { rows: RankHistoryRow[] }) {
  ...
  const data = rows.map((row) => ({ date: row.snapshot_date, rank: row.rank }))
  ...
  return (
    <ChartContainer config={chartConfig} className="aspect-auto h-40 w-full">
      <LineChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
        ...
        <ChartTooltip content={<ChartTooltipContent labelFormatter={(value) => formatShortDate(String(value))} />} />
        <Line
          dataKey="rank"
          type="monotone"
