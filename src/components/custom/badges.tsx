...
export default function Badges({ className, badges }: {
    className?: string,
    badges?: string[]
}) {
    const b : Record<string, React.ReactElement> = {
    ...
    return <div className={cn("w-fit flex flex-row items-center justify-between gap-4",className)}>
        {badges?.map((badge) => b[badge])}
    </div>;
}
