...
A lightweight Discord bot that exposes a simple HTTP API for managing user nicknames, roles, messages, and threads in a server.

## Routes

The bot exposes four POST routes. All requests must include an Authorization header and a JSON body.

### 1. `POST /set-nick`

Updates the nickname of a guild member.
...
Sends a message to a text channel in the given guild.
...
Creates a new thread in a text channel and posts the initial message.
...
Set nickname:
```bash
curl -X POST https://bot.wasans.tully.sh/set-nick \
...
Manage role:
```bash
curl -X POST https://bot.wasans.tully.sh/manage-role \
  -H "Authorization: Bearer $API_SECRET" \
  -H "Content-Type: application/json" \
...
Send message:
```bash
curl -X POST https://bot.wasans.tully.sh/send-message \
  -H "Authorization: Bearer $API_SECRET" \
  -H "Content-Type: application/json" \
...
Create thread:
```bash
curl -X POST https://bot.wasans.tully.sh/create-thread \
  -H "Authorization: Bearer $API_SECRET" \
  -H "Content-Type: application/json" \
